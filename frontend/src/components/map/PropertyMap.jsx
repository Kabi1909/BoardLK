...
function Fit({ properties, focus }) {
  const map = useMap();
  useEffect(() => {
    if (focus && properties.length) map.setView([properties[0].lat, properties[0].lng], 14);
    else map.setView([7.8731, 80.7718], 7);
  }, [focus, properties, map]);
  return null;
}
export default function PropertyMap({ properties, focus = false }) {
  ...
  return (
    ...
    >
      ...
      />
      <Fit properties={properties} focus={focus} />
      {valid.map((p) => (
