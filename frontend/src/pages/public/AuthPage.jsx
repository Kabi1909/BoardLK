...
export default function AuthPage({ mode = 'login' }) {
  ...
  const [form, setForm] = useState({
    ...
    password: '',
    confirm: '',
    role: params.get('role') === 'owner' ? 'owner' : 'renter',
    remember: false,
  });
  ...
  const titles = {
    ...
    reset: 'A fresh start for your account.',
  };
  async function submit(e) {
    ...
    if (mode === 'register' || mode === 'reset') {
      if (!passwordValid(form.password))
        errs.password = 'Use at least 8 characters, with uppercase, lowercase and a number.';
  ...
  return (
    <div className="auth-page">
      <div
        ...
        style={{
          backgroundImage: 'linear-gradient(180deg,#153c3040,#153c30e8),url(' + photos[2] + ')',
        }}
      >
        <span className="hero-pill">YOUR SPACE. YOUR STORY.</span>
        <div>
          <House size={42} />
          <h2>
            Find a place.
          ...
          <p>
            A room near campus. A fresh start in a new city.
            <br />
            Whatever comes next, make yourself at home.
          </p>
          <span>
            <ShieldCheck size={16} /> Connecting people and places across Sri Lanka
          </span>
        </div>
      </div>
      <div className="auth-form-wrap">
        <div className="auth-form">
          <span className="eyebrow">WELCOME TO BOARDLK</span>
          <h1>{titles[mode]}</h1>
          <p>
            {mode === 'login'
              ? 'Sign in to pick up where you left off.'
              : mode === 'register'
                ? 'Join a community that helps you feel at home.'
          ...
          <form onSubmit={submit} noValidate>
            ...
            {mode !== 'reset' && field('email', 'Email address', 'email')}
            ...
            {mode === 'register' && (
              <FormSelect
                ...
                options={[
                  { value: 'renter', label: 'Renter / Student' },
